...
The summary function does not handle non-numerical data well. There is a dplyr function that is part of the tidyverse that does handle categories called `count`.
...
For future reference since we used the same filter function twice we could have written that in a single line of code like this: `yrbs_filter <- filter( yrbs_select, !is.na(Sexual_identity) & Sexual_identity != "Don t know what this means")`. Now, let's try using the filter function for the `Transgender` variable:

```{r}
count(yrbs_filter, Transgender)
yrbs_filter <- filter(yrbs_select,
                      !is.na(Transgender) &
...
What if we were interested in the prevalence of an outcome instead? We could use the `summarize` function to find the prevalence of students getting bullied at school in a new column called `prevalence`. We use the `mean` function to calculate the proportion of students that responded "Yes" to the question of "During the past 12 months, have you ever been bullied on school property?" We did not filter out missing values from this variable so we need to add `na.rm=TRUE` option. The `summarize` functions return NA unless we explicitly tell the function to skip missing values. Remember NAs are not zeros!
...
```{r, solution=TRUE}
yrbs_filter %>% group_by(Transgender) %>% 
  summarize(mean(bullied == "Yes", na.rm = TRUE))
```
...
Notice how we use the `+` sign to add layers to our plot, similar to how we used `%>%` to pipe data through functions. The `aes()` function specifies our aesthetics - in this case, which variable goes on the x-axis. The `geom_bar()` automatically counts the number of observations in each category.
...
Let's visualize the bullying prevalence we calculated earlier between LGBTQ+ and non-LGBTQ+ students. First, we'll create our summary data and save it to an object we can plot.

```{r}
bullying_prev <- yrbs_filter %>% 
  group_by(LGBTQ) %>% 
...
```{r}
ggplot(bullying_prev, aes(x = LGBTQ, y = prevalence)) +
  geom_col(fill = "skyblue") +
  labs(
    title = "Prevalence of Being Bullied at School",
    x = "Student Group",
...
We notice that LGBTQ+ students have almost twice as much prevalence in experiencing bullying at school. We can also look at bullying prevalence across different demographic groups. Let's examine how it varies by race/ethnicity and LGBTQ+ status together. We'll use color to represent the LGBTQ+ grouping.
...
ggplot(race_bullying, aes(x = race, y = prevalence, fill = LGBTQ)) +
  geom_col(position = "dodge") +
  labs(
    title = "Bullying Prevalence by Race/Ethnicity and LGBTQ+ identity",
    x = "Race/Ethnicity",
    y = "Prevalence",
    fill = "Student Group"
  ) +
...
The `position = "dodge"` argument places the bars side-by-side rather than stacking them, making it easier to compare groups.
...
Now we can visualize this prevalence ratio. We'll add a horizontal line at PR = 1 to mark our reference group using `geom_hline()`.

```{r}
plot_df <- overall_prev %>%
  filter(LGBTQ == "LGBTQ+")
...
The dashed line at PR = 1 represents the reference group (Heterosexual/Not sure students). Values above this line indicate higher prevalence of bullying compared to the reference group.

## Prevalence Ratios Across Demographics

Now we can extend this analysis to see if the relationship between LGBTQ+ status and bullying varies across different demographic groups. Let's look at age first.
